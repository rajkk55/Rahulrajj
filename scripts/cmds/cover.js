...
module.exports = {
 ...
 const info = args.join(" ");
		if (!info){
			return message.reply(`Please enter in the format:\/avatar Name or code | Name | Surname`);
 ...
 const img = (`https://www.nguyenmanh.name.vn/api/avtWibu4?id=${id1}&tenchinh=${name}&tenphu=${walex}&apikey=SCn2efR7`)			
 const form = {
 ...
 const img = (`https://www.nguyenmanh.name.vn/api/avtWibu4?id=${id}&tenchinh=${name}&tenphu=${walex}&apikey=SCn2efR7`)			
 const form = {
				body: `Here's your Cover✨`
			};
				form.attachment = []
