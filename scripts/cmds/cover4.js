...
config: {
    ...
    version: "1.0",
		...
		countDown: 5,
		role: 0,
		shortDescription: "generates banner with lots of characters available",
		longDescription: "generates banner with lots of characters available",
		category: "avt & banners",
		guide: {
      en: "{pn} <character_number>|<name1>|<name2>|<name3>|<color>",
    }
	},
 ...
 onStart: async function ({ api, event, args }) {
  ...
  const color = args.join(" ").trim().replace(/\s+/g, " ").replace(/(\s+\|)/g, "|").replace(/\|\s+/g, "|").split("|")[4] || "";
    ...
    let pathImg = __dirname + `/samir/avatar_1.png`;
    let pathAva = __dirname + `/samir/avatar_2.png`;
    let avtAnime = (
    ...
    fs.writeFileSync(pathImg, Buffer.from(background, "utf-8"));
     if (!fs.existsSync(__dirname +
      ...
      fs.writeFileSync(__dirname + `/samir/PastiOblique-7B0wK.otf`, Buffer.from(getfon2t, "utf-8"));
    };
         if (!fs.existsSync(__dirname +
      `/samir/gantellinesignature-bw11b.ttf`)) {
      ...
      fs.writeFileSync(__dirname + `/samir/gantellinesignature-bw11b.ttf`, Buffer.from(getfon3t, "utf-8"));
    };
        if (!fs.existsSync(__dirname +
      `/samir/UTM%20Bebas.ttf`)) {
      ...
      fs.writeFileSync(__dirname + `/samir/UTM%20Bebas.ttf`, Buffer.from(getfon3t2, "utf-8"));
    };
    ...
    ctx.drawImage(a, 0, 0, canvas.width, canvas.height);
     ...
     ctx.textAlign = "start";
  Canvas.registerFont(__dirname + `/samir/PastiOblique-7B0wK.otf`, {
    ...
    ctx.textAlign = "start";
  Canvas.registerFont(__dirname + `/samir/gantellinesignature-bw11b.ttf`, {
    ...
    ctx.save();
     Canvas.registerFont(__dirname + `/samir/UTM%20Bebas.ttf`, {
    family: "Bebas"
  });
    ctx.textAlign = "end";
